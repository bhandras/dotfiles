-- My personal neovim config, based on https://github.com/nvim-lua/kickstart.nvim
...
vim.keymap.set("n", "<space>", "za", { desc = "Fold" })
...
local function ordered_remote(preferred)
...
require("lazy").setup({
	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set("n", "<leader>1", ":BufferLineGoToBuffer 1<CR>", { desc = "Switch to buffer 1" })
			vim.keymap.set("n", "<leader>2", ":BufferLineGoToBuffer 2<CR>", { desc = "Switch to buffer 2" })
			vim.keymap.set("n", "<leader>3", ":BufferLineGoToBuffer 3<CR>", { desc = "Switch to buffer 3" })
			vim.keymap.set("n", "<leader>4", ":BufferLineGoToBuffer 4<CR>", { desc = "Switch to buffer 4" })
			vim.keymap.set("n", "<leader>5", ":BufferLineGoToBuffer 5<CR>", { desc = "Switch to buffer 5" })
			vim.keymap.set("n", "<leader>6", ":BufferLineGoToBuffer 6<CR>", { desc = "Switch to buffer 6" })
			vim.keymap.set("n", "<leader>7", ":BufferLineGoToBuffer 7<CR>", { desc = "Switch to buffer 7" })
			vim.keymap.set("n", "<leader>8", ":BufferLineGoToBuffer 8<CR>", { desc = "Switch to buffer 8" })
			vim.keymap.set("n", "<leader>9", ":BufferLineGoToBuffer 9<CR>", { desc = "Switch to buffer 9" })
		end,
	},
	{
		"folke/trouble.nvim",
	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set(
				"n",
				"<leader>no",
				":Neotest output-panel toggle<CR>",
				{ desc = "[N]eotest Toggle [O]utput Panel" }
			)
		end,
	},
	require("gitlinker").setup({
		remote = ordered_remote({ "upstream", "origin" }),
		callbacks = {
...
vim.api.nvim_create_autocmd("FileType", {
	pattern = "go",
	callback = function()
		-- Enable proper comment formatting
		-- 't' = auto-wrap text using textwidth
		-- 'c' = allow formatting of comments
		...
		-- Disable LSP or plugin-based format expressions
		vim.opt_local.formatexpr = ""

		-- Disable external format program (like gofmt)
		vim.opt_local.formatprg = ""

		-- Explicitly define comment styles for Go
		-- Handles both // and /* */ style comments
		vim.opt_local.comments = "s1:/*,mb:*,ex:*/,://"

		-- If 'gq' is mapped in visual mode (e.g., to !gofmt), remove it
